...
|26|通过软盘拷贝文件|动态规划|200| ***01背包
|27|代码编辑器|逻辑分析|200| 简单，略过
|28|数据分类|进制 / 位运算|100| *****
|29|恢复数字序列|滑动窗口|100|
...
|34|字符串花间化繁为简|字符串|200| *没有题解
|35|数据最节约的备份方法|二分法|100| ***
|36|跳格子2|逻辑分析|100| *半^
|37|跳房子2|数据结构，排序？|200| ***三数之和
|38|支持优先级的队列|数据结构 / 优先队列PQ|100| *****
|39|乘坐保密电梯|回溯法/递归|100| *****抄
|40|计算最接近的数|逻辑分析 / 滑动窗口|100| *抄
|41|人气最高的店铺|逻辑分析|100| *****抄
|42|战场索敌|DFS|200| *****典型DFS
|43|二维伞的雨滴效应|二叉树|100| ![img_1.png](img_1.png) *****
|44|跳房子1I|数据结构|100| ![img_2.png](img_2.png) ![img_3.png](img_3.png) 
---
|45|分割数组的最大差值|逻辑分析|100| 简单，略过
|46|生日礼物|二分法|100|  *没有题解
|47|求最小步数|逻辑分析|100| * 背  
|48|拔河比赛|数据结构|100| ^
|49|评论转换输出|回溯法/递归|200| *没有题解
|50|树状结构查询|数据结构/树|200| 简单，略过[HashMap树,k=右v=左]

|51|不开心的小朋友|逻辑分析|200| * 背 [一个HashSet记录当前玩的，一个Queue记录排队的]
|52|数字序列比大小|逻辑分析|200| LC#455分发饼干 [田忌赛马：两序列排序号遍历A找B输的最大值]
|53|符合要求的元组个数/K数之和|回溯法/递归|100| *没有题解
|54|最小循环数组|KMP|200| 没有KMP？ [数组拼接成字符串，遍历数组公约数(长度%i)，replace成空串]
|55|金字塔/微商的收入|数据结构 / 自定义 排序算法|100|
|56|矩阵元素边界值|字符串处理|100| 简单，略过
|57|MELON的难题|动态规划|200| ***01背包
|58|矩阵中非1的元素个数|BFS|200| *****BFS的典型
|59|计算误码率|逻辑分析|200| *****逻辑简单，实现复杂————————————————————————————————
|60|增强的strstr|正则匹配|100|

|61|关联子串|字符串处理|100| * 背
|62|计算礼品发放的最小分组数目|双指针|100|
|63|字符匹配|正则匹配|100|
|64|最小传输时延I|有源最短路径|200| *****Dijkstra狄克斯特拉算法—————————————————————
...
|77|荒岛求生|数据结构 / 栈|200|          ***单调栈
|78|查字典|字符串处理|100| 简单，略过
|79|周末爬山|BFS|200|     重要 但看不懂 *****广度优先/图遍历/BFS
|80|最小数字|排序算法|100| 简单，略过[字符串拼接比大小]

|81|最长的元音子串|字符串|100| 简单，略过[两个列表比较]
|82|TLV解析II|数据结构|100| 复杂，弃
|83|玩牌高手|逻辑分析|200| ^
|84|最长连续子序列|滑动窗口|100|
|85|路灯照明II|区间合并|100| 未刷，基本会做
|86|最多团队|排序算法|100| 未刷，基本会做[两个指针向中间移动]
|87|最长的顺子|数据结构|100| 复杂，弃
|88|数字最低位排序|排序算法|100| 未刷，基本会做
|89|整数编码|字符串 / 进制|100| 
|90|找车位|逻辑分析|100| 未刷，基本会做[将车位作为索引表]
|91|选举拉票|线段树|200| 很难，弃
|92|单词加密|字符串|100| 未刷，基本会做
